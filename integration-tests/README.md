...
# 2. Publish the module (Cargo.toml pins the crates.io bindings; match your server)
...
Against SpacetimeDB **2.10.0** (CLI 2.10.0), schema v10, Rust modules built against the
crates.io `spacetimedb = "2.10.0"` bindings and the TypeScript module against npm
`spacetimedb@2.10.0`:
...
- Enum-with-payload: 1/1. Anonymous `Result<T, E>`: 2/2. PK-less refcount: 3/3. Reconnect identity: 1/1. Submodules: 8/8.
...
**Last verified (2.10.0):** `shape` → `value=0` (Circle), `data=7`. Write path
...
`ENUM_OPTIONS [i32, string]`), so it rides the same enum path. **Last verified (2.10.0):**
...
cd integration-tests/schema_probe && cargo run --release > schema.json
# stdout = the schema JSON; stderr = the namespaced names the engine registers.
```

Edit `src/main.rs` to build whatever module shape you need to inspect.
