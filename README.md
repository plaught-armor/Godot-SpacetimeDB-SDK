...
-   **Typed per-field finders:** each table wrapper also generates `find_by_<field>(value)` / `first_by_<field>(value)` for every scalar field — a compile-checked field name and value type and a typed return, instead of the stringly-typed `find_by(&"field", value)`.
