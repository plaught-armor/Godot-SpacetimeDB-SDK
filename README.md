...
> Requires **SpacetimeDB 2.2.0+** (v3 BSATN protocol with batched framing, schema v10) and **Godot 4.6+** (the editor plugin uses `EditorDock`, added in 4.6). Tested with `SpacetimeDB 2.2.0`–`2.8.3` and Godot `4.6-stable`, `4.7-stable` and `4.8.dev`. The legacy v2 sub-protocol was dropped in 2.0; for servers below 2.2.0 use an SDK `1.x` release.
...
-   **Extended scalar types:** `i128` / `u256` / `i256` (raw `PackedByteArray`), `Uuid` (reuses the `u128` wire path), and `ScheduleAt` (the `Interval | Time` tagged union on `#[scheduled]` tables, exposed as a `ScheduleAt` resource). Verified byte-exact end-to-end against a live SpacetimeDB 2.8.3 server (see [`integration-tests/`](integration-tests/)).
