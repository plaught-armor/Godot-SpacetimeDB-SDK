...
> Requires **SpacetimeDB 2.2.0+** (v3 BSATN protocol with batched framing, schema v10) and **Godot 4.6+** (the editor plugin uses `EditorDock`, added in 4.6). Tested with `SpacetimeDB 2.2.0`–`2.10.0` and Godot `4.6-stable`, `4.7-stable` and `4.8.dev`. The legacy v2 sub-protocol was dropped in 2.0; for servers below 2.2.0 use an SDK `1.x` release.
...
-   **Handles survive an auto-reconnect:** a drop suspends a `SpacetimeDBSubscription` (`suspended` true, `active` false) and the reconnect re-registers that same handle under a fresh query set id — `applied` fires again and `unsubscribe()` keeps working. `end` is reserved for a subscription that really ended.
...
-   **Query Builder:** `SpacetimeDBQuery.table("user").where("online", true).to_sql()` — fluent API with SQL identifier validation and auto-escaping for strings, booleans, and identities. Also `where_in(field, values)` and `where_any([[f, v], ...])`, both emitted as OR groups (SpacetimeDB SQL has no `IN` operator).
...
-   **Typed per-field finders:** each table wrapper also generates `find_by_<field>(value)` / `first_by_<field>(value)` for every value-typed scalar field (`int`/`float`/`String`/`bool`/`StringName`/`PackedByteArray`) — a compile-checked field name and value type and a typed return, instead of the stringly-typed `find_by(&"field", value)`. A finder for a field backed by a **unique index** routes through that index's O(1) cache, a **btree index** field through its multimap cache (O(1)+k); other fields use the linear scan.
...
-   **Namespaced submodules (SpacetimeDB 2.8.1+):** a module that registers other modules under namespaces generates bindings for all of them. A submodule's tables and functions sit under a namespace member mirroring the dotted name the server registers them under — `lib.lib_data` is `db.lib.lib_data`, `auth.baz.baz_insert` is `reducers.auth.baz.baz_insert(...)` — with each submodule's types scoped to its own typespace. Verified end-to-end against a live 2.10.0 server (see [`integration-tests/`](integration-tests/) and [Submodules](docs/submodules.md)).

### Connection & Reliability

-   **Auto-Reconnection:** Exponential backoff with jitter, configurable via `SpacetimeDBConnectionOptions`. Signals: `reconnecting`, `reconnected`, `reconnect_failed`. Subscription queries are automatically restored on reconnect, on the caller's own handles — a `SpacetimeDBSubscription` is suspended by the drop and re-registered by the reconnect, so it keeps working (including `unsubscribe()`). Reducer and procedure handles in flight are stamped `DISCONNECTED`, and every handle is ended on a terminal disconnect. Regaining application focus fires a pending attempt immediately (`reconnect_on_app_resume`, on by default) — a backgrounded frame loop is throttled or stopped, which stalls the backoff timer, so a drop that happens off-screen would otherwise outlive the return to the app.
...
-   **Confirmed reads:** `SpacetimeDBConnectionOptions.confirmed_reads` waits for durable commit before the server sends an update (on by default, matching the server's own default for a v3 connection — set it `false` to trade durability for latency).
...
-   **Extended scalar types:** `i128` / `u256` / `i256` (raw `PackedByteArray`), `Uuid` (reuses the `u128` wire path), and `ScheduleAt` (the `Interval | Time` tagged union on `#[scheduled]` tables, exposed as a `ScheduleAt` resource). Verified byte-exact end-to-end against a live SpacetimeDB 2.10.0 server (see [`integration-tests/`](integration-tests/)).
...
-   **A message the server sends immediately before closing may never arrive — the engine drops it.** Godot's `WebSocketPeer` clears its inbound queue the moment a clean close completes, and the close handshake completes inside a single `poll()`, so a data frame that arrived in the same read is discarded before any script can drain it (`in_buffer.clear()` in `close()`, `modules/websocket/wsl_peer.cpp`). SpacetimeDB flushes its pending frames right before the close frame, which is exactly the arrangement that loses them. Nothing the SDK can do — it never sees the bytes. Reproducer: `godot-client/tests/_repro_ws_close_drops_final_messages.gd`. Full rationale: [Design Decisions](docs/design-decisions.md#blocked-by-the-engine--not-actionable-client-side).
-   **Reducer-event caller identity, connection id, and energy are not exposed — they are not on the wire.** SpacetimeDB stripped these from `TransactionUpdate` at the v1 protocol cut, so no client SDK (Rust / C# / TypeScript / Godot) can surface them. A row callback fired by *another* client's reducer cannot know who caused it; your *own* call is recoverable by correlating `request_id` (which `SpacetimeDBReducerCall` already does). Full rationale: [Design Decisions](docs/design-decisions.md#blocked-by-the-wire--not-actionable-client-side).
-   **No "out of energy" reducer outcome — it folds into an error.** The v2/v3 `ReducerOutcome` has no distinct out-of-energy variant; energy exhaustion arrives as `InternalError` / `Err` with a message. Inspect the error text — there is no enum value to branch on. Full rationale: [Design Decisions](docs/design-decisions.md#blocked-by-the-wire--not-actionable-client-side).
-   **Reducer call flags (`NoSuccessNotify` / `FullUpdate`) are not supported — the wire accepts only `Default`.** v2/v3 `CallReducerFlags` is single-variant; the fire-and-forget flags existed in v1 and were removed, and no SDK offers per-call flag tuning. Full rationale: [Design Decisions](docs/design-decisions.md#blocked-by-the-wire--not-actionable-client-side).
