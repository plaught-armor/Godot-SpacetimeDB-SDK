...
-   **Indexed lookups:** unique indexes expose a typed `find(value)` (single row); non-unique btree indexes expose a typed `filter(value)` (all matching rows), generated as named accessors on the table wrapper. A btree index over an **orderable** column (`int` / `float` / `String`) also gets `filter_range(from, to)` returning every row whose key is in `[from, to]` inclusive — backed by a sorted-key mirror, so it binary-searches the window (O(log d + k)) instead of scanning. Bytes-backed keys (`Identity`, `u128`/`u256`) get exact-match `filter()` only, since they have no defined ordering.
