...
- **Update detection on a generated row is about 40% faster. Regenerate your bindings to
  ...
  record. Codegen now emits that comparison typed to each row's own columns: a static
  `_eq` on every record type and a `_row_eq` override on every row type, which
  `LocalDatabase` calls for change detection and for matching rows in a table without a
  primary key. It answers exactly what the walk answers, NaN and null included. The new
  `test_typed_row_equality.gd` holds the two together for every column shape codegen
  emits, and each check arm broken on purpose fails it. Per comparison the generated
  override is 2.0–3.6× faster (`bench_rows_equal.gd`, circle row 1725 → 490 ns). The
  update wave in `bench_apply_profile.gd` fell 3333 → 2012 ns/row on the entity row and
  3436 → 2098 on the circle row, and 3044 → 1679 / 3038 → 1831 on the Godot 4.7 release
  template (N=100k, best-of-7, median of 5 rounds).
  Bindings generated by an earlier version keep working. A row without the override falls
  back to the walk through one extra call, which makes its updates about 6% slower
  (2136 → 2261 ns/row) until you regenerate. A column named `_eq` is now escaped like the
...
> `SpacetimeDBSubscription` handle now survives an auto-reconnect, so a transient drop no
> longer emits `end` — the handle suspends and resumes, and `end` now means the
> subscription really ended. Code that watched `end` to notice a drop should listen to the
> client's `reconnecting` signal instead, and code that took a fresh handle in response to
> `reconnected` should keep the one it already holds: subscribing again duplicates the
> query set server-side. See "A subscription handle now survives an auto-reconnect" below.
>
> Separately, `SpacetimeDBConnection.close_diagnostic()` takes the close reason as a third
> argument. Nothing in the SDK's own surface calls it — a game that does must pass
> `WebSocketPeer.get_close_reason()` along with the code.
