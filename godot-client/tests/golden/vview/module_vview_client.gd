...
@tool
class_name VviewModuleClient extends SpacetimeDBClient

var reducers: VviewModuleReducers
var procedures: VviewModuleProcedures
var db: VviewModuleDb

func _init() -> void:
	module_name = "Vview"
	name = "VviewModule"
	reducers = preload('user://golden_gen/vview/module_vview_reducers.gd').new(self)
	procedures = preload('user://golden_gen/vview/module_vview_procedures.gd').new(self)

func _init_db(p_local_db: LocalDatabase) -> void:
	db = preload('user://golden_gen/vview/module_vview_db.gd').new(p_local_db)
