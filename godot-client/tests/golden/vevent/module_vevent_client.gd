...
@tool
class_name VeventModuleClient extends SpacetimeDBClient

var reducers: VeventModuleReducers
var procedures: VeventModuleProcedures
var db: VeventModuleDb

func _init() -> void:
	module_name = "Vevent"
	name = "VeventModule"
	reducers = preload('user://golden_gen/vevent/module_vevent_reducers.gd').new(self)
	procedures = preload('user://golden_gen/vevent/module_vevent_procedures.gd').new(self)

func _init_db(p_local_db: LocalDatabase) -> void:
	db = preload('user://golden_gen/vevent/module_vevent_db.gd').new(p_local_db)
