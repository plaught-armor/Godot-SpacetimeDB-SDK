...
@tool
class_name VschedModuleClient extends SpacetimeDBClient

var reducers: VschedModuleReducers
var procedures: VschedModuleProcedures
var db: VschedModuleDb

func _init() -> void:
	module_name = "Vsched"
	name = "VschedModule"
	reducers = preload('user://golden_gen/vsched/module_vsched_reducers.gd').new(self)
	procedures = preload('user://golden_gen/vsched/module_vsched_procedures.gd').new(self)

func _init_db(p_local_db: LocalDatabase) -> void:
	db = preload('user://golden_gen/vsched/module_vsched_db.gd').new(p_local_db)
