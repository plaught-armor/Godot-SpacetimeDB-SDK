...
## a dictionary cache that stays in sync with [LocalDatabase] via the insert/update/delete
## hooks it registers, which run while a message is applied.
...
## because neither wipe goes through the hooks above — this cache would otherwise keep
...
## Update hook — moves the row to its new key when the key changed. The old key is
...
## reports a message's changes to this hook in no particular row order.
...
## Delete hook — drops the row's key from the cache, unless the key has already been
