...
const REQUIRED_FOLDERS_IN_CODEGEN_FOLDER: Array[String] = ["tables", "types"] # gdlint: ignore[S6] — C1: no `const Packed*`
...
const ORDERABLE_INDEX_TYPES: Array[String] = ["int", "float", "String"] # gdlint: ignore[S6] — C1: no `const Packed*`
## Column types a generated `_eq` compares with `!=`, keyed to their Variant type so
## [constant LocalDatabase._NAN_CARRYING_COMPONENTS] decides which of them also need the
## NaN test. A column of any other type — Option, array, sum type, ScheduleAt — compares
## through [method LocalDatabase._values_equal], which already descends into it.
const _EQ_VALUE_TYPES: Dictionary[String, int] = {
	"int": TYPE_INT,
	"bool": TYPE_BOOL,
	"float": TYPE_FLOAT,
	"String": TYPE_STRING,
...
const GDSCRIPT_RESERVED_WORDS: Array[String] = [ # gdlint: ignore[S6] — C1: no `const Packed*`
	"namespace",
	"trait",
	"if",
	"elif",
...
static func _build_bsatn_type(nested_type: Array, base_meta_type: String, skip_outermost: bool = true) -> String:
	...
	for i: int in range(nested_type.size() - 1, start_idx - 1, -1):
...
## `BSATN_TYPES` / `create` / `_eq`; the table wrapper spells the three typed change
## signals. Everything else those classes carry is inherited, and
...
func _param_to_bsatn_type(schema: SpacetimeParsedSchema, param: Dictionary) -> String:
	...
	if not p_nested.is_empty():
		...
		if param_type and param_type.has("gd_arraylike"):
			...
			for el: Dictionary in param_type.struct:
...
static func module_prefix_problems(module_names: PackedStringArray) -> PackedStringArray:
...
func generate_bindings() -> PackedStringArray:
	...
	var sorted_module_names: PackedStringArray = _plugin_config.module_configs.keys()
...
func _generate_module_bindings(module_name: String) -> PackedStringArray:
	...
	for folder: String in REQUIRED_FOLDERS_IN_CODEGEN_FOLDER:
	...
	if file == null:
		...
		file.close()
	_stage_reached_return = false
	var generated_files: PackedStringArray = _generate_gdscript_from_schema(module_name, schema)
	if not _stage_reached_return:
		SpacetimePlugin.print_err(
			(
...
func _generate_gdscript_from_schema(module_name: String, schema: SpacetimeParsedSchema) -> PackedStringArray:
	var generated_files: PackedStringArray = []
	...
	for type_def: Dictionary in schema.types:
		...
		if type_def.has("struct"):
			var generated_table_names: PackedStringArray
	...
	for table_def: Dictionary in schema.tables:
		...
		for unique_index: Dictionary in unique_indexes:
		...
		for btree_index: Dictionary in btree_indexes:
...
func _generate_table_unique_index_gdscript(schema: SpacetimeParsedSchema, unique_index_def: Dictionary, table_def: Dictionary) -> String:
	...
	# An index on the table's primary key reads LocalDatabase's own table, which is already
	# primary key -> row: a cache of its own would be a second copy kept current by a hook
	# call on every changed row. Same per-table source LocalDatabase keys the table by.
	if field_name == _safe_field_name(table_def.get("primary_key_name", "")):
...
func _generate_table_gdscript(
	...
	# Callable API). LocalDatabase emits them directly, registered in _init.
	out.append(
...
func _table_scalar_fields(schema: SpacetimeParsedSchema, type_def: Dictionary) -> Array:
	...
	const VALUE_TYPES: Array[String] = ["int", "float", "String", "bool", "StringName", "PackedByteArray"] # gdlint: ignore[S6] — C1: no `const Packed*`
	var out: Array = []
...
func _generate_struct_gdscript(schema: SpacetimeParsedSchema, type_def: Dictionary, table_names: PackedStringArray) -> String:
	...
	# PackedStringArray has no map(); the loop is the packed-container equivalent.
	var table_name_literals: PackedStringArray = []
	for name: String in table_names:
		table_name_literals.append(_format_table_name_literal(name))
	var header: String = (AUTOGENERATED_COMMENT +
			...
			[", ".join(table_name_literals)])

	var class_fields: Array = []
	var eq_checks: String = ""
	var row_eq_checks: String = ""
	...
	for i: int in fields.size():
		...
		if not parser_nested_type.is_empty():
			...
			if field_type and field_type.has("gd_arraylike"):
				var inner_meta_bsatn_types: PackedStringArray = []
				for el: Dictionary in field_type.struct:
		...
		# `_row_eq` reads this row's columns through `self.`, so a column named like the local
		# holding the other row still reads the column.
		var eq_type_def: Dictionary = field_type if field_type is Dictionary else { }
		var eq_wrapped: bool = not parser_nested_type.is_empty()
		eq_checks += _eq_check_lines("p_lhs." + field_name, "p_rhs." + field_name, gd_field_type, eq_type_def, eq_wrapped)
		row_eq_checks += _eq_check_lines("self." + field_name, "p_rhs." + field_name, gd_field_type, eq_type_def, eq_wrapped)
	...
	for field_data: Array in class_fields:
		var f_name: String = field_data[0]
		out.append("\tresult.%s = p_%s\n" % [f_name, f_name])
	out.append("\treturn result\n")
	out.append(_generate_eq_gdscript(_class_name, eq_checks, row_eq_checks, _extends_class == "_ModuleTableType"))
	return "".join(out)


## The value comparison emitted on one record type: a static `_eq` running [param checks],
## and on a row type the [method _ModuleTableType._row_eq] override LocalDatabase's change
## detection calls, running [param row_checks]. The override repeats the checks rather than
## calling `_eq`: that one extra call measured ~30% of a whole row comparison.
static func _generate_eq_gdscript(record_class: String, checks: String, row_checks: String, is_row: bool) -> String:
	var out: String = (
		"\n\n## Whether two %s values are equal column by column, as LocalDatabase compares rows.\n" % record_class
		+ "static func _eq(p_lhs: %s, p_rhs: %s) -> bool:\n" % [record_class, record_class]
...
## One column's test inside a generated `_eq` or `_row_eq`: returns false when [param lhs]
## and [param rhs], the two expressions reading that column, differ. It must answer what
## [method LocalDatabase._rows_equal] answers for that column — two NaNs are equal, a
## nested record compares by value — or update detection changes meaning between generated
## and hand-written rows (tests/test_typed_row_equality.gd). [param gd_type] is the
## column's declared type, [param type_def] the parsed schema type it names (empty for a
## primitive), and [param wrapped] whether an Option or an array wraps it.
static func _eq_check_lines(lhs: String, rhs: String, gd_type: String, type_def: Dictionary, wrapped: bool) -> String:
	var differs: String = "not LocalDatabase._values_equal(%s, %s)" % [lhs, rhs]
	# A plain enum column holds an int. A wrapped column, a sum type and a record with no
	# columns keep the _values_equal call: a record without columns has no BSATN_TYPES, so
	# the walk compares it by identity.
	var check_type: String = gd_type
	if type_def.has("enum") and not type_def.get("is_sum_type", true):
		check_type = "int"
	if wrapped:
		pass
	elif type_def.has("struct") and not type_def.has("gd_native") and not (type_def["struct"] as Array).is_empty():
		differs = "not %s._eq(%s, %s)" % [gd_type, lhs, rhs]
	elif _EQ_VALUE_TYPES.has(check_type):
		var variant_type: int = _EQ_VALUE_TYPES[check_type]
		var components: int = LocalDatabase._NAN_CARRYING_COMPONENTS.get(variant_type, 0)
		if variant_type == TYPE_FLOAT:
			differs = "%s != %s and not (is_nan(%s) and is_nan(%s))" % [lhs, rhs, lhs, rhs]
		elif components > 0:
			differs = (
				"%s != %s and not LocalDatabase._nan_components_equal(%s, %s, %d)"
				% [lhs, rhs, lhs, rhs, components]
			)
		else:
			differs = "%s != %s" % [lhs, rhs]
	return "\tif %s:\n\t\treturn false\n" % differs
...
func _generate_enum_gdscript(schema: SpacetimeParsedSchema, type_def: Dictionary) -> String:
	...
	var get_funcs: PackedStringArray = []
	var create_funcs: PackedStringArray = []
	for v_schema: Dictionary in variants:
...
func _generate_types_gdscript(module_name: String, schema: SpacetimeParsedSchema) -> String:
	...
	for _type_def: Dictionary in schema.types:
		...
		if _type_def.has("is_sum_type") and not _type_def.get("is_sum_type"):
			...
			for variant: Dictionary in _type_def.get("enum", []):
...
func _generate_reducers_gdscript(module_name: String, schema: SpacetimeParsedSchema) -> String:
	...
	for reducer: Dictionary in schema.reducers:
		...
		var params_str_parts: PackedStringArray = []
		var description_comment: Array = []
		var reducer_params = reducer.get("params", [])
		for i: int in reducer_params.size():
		...
		for p: Dictionary in reducer.get("params", []):
...
func _generate_procedures_gdscript(_module_name: String, schema: SpacetimeParsedSchema) -> String:
	...
	for proc: Dictionary in schema.procedures:
		var params_str_parts: PackedStringArray = []
		var description_comment: Array = []
		var proc_params = proc.get("params", [])
		for i: int in proc_params.size():
		...
		var param_bsatn_types_list: Array = []
		for p: Dictionary in proc.get("params", []):
			param_bsatn_types_list.append(_param_to_bsatn_type(schema, p))
		var param_bsatn_types_str: String = ""
		if not param_bsatn_types_list.is_empty():
			param_bsatn_types_str = ", ".join(param_bsatn_types_list)
