...
**C1. `const` packed arrays are not read-only** ([#88753](https://github.com/godotengine/godot/issues/88753) for the older read-back symptom). The tracker symptom — `const Array[Packed*Array]` reporting a byte-count `.size()` and reading 0.0 — does NOT reproduce on 4.6.stable or 4.8.dev; every const packed shape reads back its authored contents. What is live on both: a `const Packed*Array` is not read-only and has no `is_read_only()`/`make_read_only()` to ask or lock. Bind it to a local, append, and the constant is rewritten process-wide with no error, visible to a later independent binding — where a `const Array[T]`/`const Dictionary` raises "Array is in read-only state". Use `static var` for class-accessed, `var` otherwise; and when a shared table must genuinely be frozen, `Array[T]` + `make_read_only()` is the container that can be, which is the one case it beats the packed type (cf. S6).

**C2. `const` arrays/dicts were mutable shared refs** ([#61274](https://github.com/godotengine/godot/issues/61274)). Measured on 4.6.stable and 4.8.dev: a `const` `Array`/`Dictionary` is read-only, so `.append()` on it raises instead of mutating globally. `Packed*Array` is the exception and still shares silently (C1). Discipline unchanged: never mutate a `const` container; `.duplicate()` first or use `var`.
...
**P19. Don't wrap a named fn in a pass-through lambda** (advisory, `dod.md` D9). `func(x): return f(x)` double-dispatches (~6.5× inline) vs passing the reference `f` / `obj.method` directly as a `Callable` (~3.8×) — ~1.7× cost for nothing. Pass the reference. A bare lambda used as a predicate/comparator is fine (~3.6× tier; capturing a local adds ~10%). Flag: a lambda whose entire body is a single call forwarding its args to one named function.
